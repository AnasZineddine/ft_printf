...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_print_capx_norme.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: azineddi <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/25 16:05:56 by azineddi          #+#    #+#             */
/*   Updated: 2019/11/02 16:32:32 by azineddi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/ft_printf_header.h"

void	calculate_spaces_to_print_capx(t_format *strc, t_var *var_d)
{
	...
	precision && !ft_strchr(strc->flags, '0'))
		var_d->spaces_to_print = strc->field_width - var_d->
	len - var_d->sign_char;
}

void	ft_print_without_minus_capx(uintmax_t n, t_format *strc, t_var *var_d)
{
	if (!ft_strchr(strc->flags, '-'))
	{
		...
		if (n == 0 && strc->precision != -2 && strc->precision != -3)
			...
			ft_printubase(n, 16, 0);
	}
}

void	ft_print_result_w_minus_capx(uintmax_t n, t_format *strc, t_var *var_d)
{
	if (ft_strchr(strc->flags, '-'))
	{
		if (var_d->print_minus_check)
		...
		ft_print_without_minus_capx(n, strc, var_d);
}
...
int		ft_printf_capx(uintmax_t n, t_format *strc)
{
	...
	ft_initialize_to_zero_capx(&var_d);
	count_len_capx(n, &var_d, strc);
	check_flags_one_char_capx(n, strc, &var_d);
	calculate_zeros_to_print_capx(strc, &var_d);
	calculate_spaces_to_print_capx(strc, &var_d);
	...
	ft_print_result_w_minus_capx(n, strc, &var_d);
