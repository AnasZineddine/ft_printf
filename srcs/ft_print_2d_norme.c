...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/25 16:10:06 by azineddi          #+#    #+#             */
/*   Updated: 2019/11/02 16:41:12 by azineddi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (strc->field_width > var_d->len && ft_strchr(strc->flags, '-') &&
strc->precision >= var_d->len && strc->field_width > strc->precision)
...
void	ft_print_without_minus_d(intmax_t n, t_format *strc, t_var *var_d)
{
...
void	ft_print_result_with_minus_d(intmax_t n, t_format *strc, t_var *var_d)
{
	if (ft_strchr(strc->flags, '-'))
	{
		if (var_d->print_minus_check)
		...
		ft_print_without_minus_d(n, strc, var_d);
}
...
int		ft_printf_d(intmax_t n, t_format *strc)
{
	...
	ft_initialize_to_zero_d(&var_d);
	count_len_d(n, &var_d, strc);
	check_flags_one_charachter_d(n, strc, &var_d);
	calculate_zeros_to_print_d(strc, &var_d);
	calculate_spaces_to_print_d(strc, &var_d);
	...
	ft_print_result_with_minus_d(n, strc, &var_d);
